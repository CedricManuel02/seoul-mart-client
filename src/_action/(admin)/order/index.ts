...
export async function createShippedServerAction(payload: { order_number: string; shipped_details: any }) {
  const auth_token = await getSessionNextAuth();
  const formData = new FormData();

  formData.append("delivery_company", payload.shipped_details.delivery_company);
  formData.append("delivery_rider_name", payload.shipped_details.delivery_rider_name);
  formData.append("delivery_rider_phone", payload.shipped_details.delivery_rider_phone);
  formData.append("delivery_plate_number", payload.shipped_details.delivery_plate_number);
  formData.append("delivery_tracking_number", payload.shipped_details.tracking_number);
  ...
  const response = await fetch(`${API_ENDPOINT}/auth/shipped/${payload.order_number}`, {
    method: "POST",
    headers: {
      Cookie: `auth__token=${auth_token}`,
    },
