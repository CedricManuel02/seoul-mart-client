    ...
    <Dialog onOpenChange={setIsOpen} open={isOpen}>
      ...
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>Here's the invoice receipt for the order.</DialogDescription>
        </DialogHeader>
        <PDFViewer className="w-full h-[550px] hidden lg:block">
          <MyDocument order={order} />
        </PDFViewer>
        <DialogFooter>
          <PDFDownloadLink document={<MyDocument order={order} />} fileName={`order-${order.order_number}.pdf`}>
